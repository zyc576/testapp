...
using ZTunnel.Pmms.Core.Attributes;

namespace ZTunnel.Pmms.Core.Dapper
{
    public static class DapperHelper
    {
        ...
        public static DataTable GetTablePaged(this IDbConnection connection, int pageNumber, int rowsPerPage, string tableName, string cols, string conditions, string orderby, object parameters = null, int? commandTimeout = null)
        {
            if (string.IsNullOrEmpty(_getPagedListSql))
                throw new Exception("GetListPage is not supported with the current SQL Dialect");
            ...
            var name = tableName;
            var query = _getPagedListSql;
            query = query.Replace("{SelectColumns}", cols.ToString());
            query = query.Replace("{TableName}", name);
            query = query.Replace("{PageNumber}", pageNumber.ToString());
            query = query.Replace("{RowsPerPage}", rowsPerPage.ToString());
            query = query.Replace("{OrderBy}", orderby);
            ...
            if (Debugger.IsAttached)
                Trace.WriteLine(String.Format("GetListPaged<{0}>: {1}", "table", query));

            var ds = connection.ExecuteReader(query, parameters, null, commandTimeout);
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static int? Insert<TEntity>(this IDbConnection connection, List<TEntity> entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var name = GetTableName(entityToInsert[0]);
            var sb = new StringBuilder();
            sb.AppendFormat("insert into {0}", name);
            sb.Append(" (");
            BuildInsertParameters<TEntity>(sb);
